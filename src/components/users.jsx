...
const Users =()=>{
    ...
    const getSpanContent=()=> {
          return (counter>0?`${counter} человек 
                 ${(counter%10>=2 && counter%10<=4 && (counter<10 || counter>20) ?'а':'')}
                 тусанет с тобой сегодня`:`Сегодня никто не тусанет с тобой`)
    ...
    const renderTable=()=>{
        return <table 
                  className={(counter<1?'hide':
                                        'table table-dark table-striped table-responsive')}>
                  <thead>
                    <tr>
                      {renderCaption()}
                    </tr>
    ...
    const renderCaption=()=> {
        return ['Имя','Качества','Профессия',
                'Встретился, раз','Оценка','Действие']
                .map((val, id)=>(<th scope="col" key={id}>{val}</th>))  
    }

    const renderQualities=(qualities)=> {
       //console.log('test of quality')
        return qualities.map((quality,id,res)=>
    ...
    const renderUsers=()=>{
        return Api.users.fetchAll().map((user,id)=>
              <tr key={id+1} data-id={user._id}>  
              ...
              <td key={(id+1)*100+5}><button className='btn btn-danger m-2' 
